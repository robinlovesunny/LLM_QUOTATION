...
SERVICE_NAME="llm-quotation-backend"
SERVICE_FILE="${SCRIPT_DIR}/${SERVICE_NAME}.service"
SYSTEMD_DIR="/etc/systemd/system"
LOGROTATE_FILE="${SCRIPT_DIR}/llm-quotation-logrotate"
LOGROTATE_DIR="/etc/logrotate.d"
HEALTH_URL="http://localhost:8000/health"
HEALTH_DETAILED_URL="http://localhost:8000/health/detailed"
...
install_gunicorn() {
    ...
    if [ -d "${BACKEND_DIR}/venv" ]; then
        ...
        print_success "Gunicorn 已就绪"
    else
        print_error "虚拟环境不存在，请先运行 start.sh"
        exit 1
    fi
}
...
install_service() {
    ...
    if [ -f "$LOGROTATE_FILE" ]; then
        cp "$LOGROTATE_FILE" "${LOGROTATE_DIR}/llm-quotation"
...
uninstall_service() {
    ...
    # 删除日志轮转配置
    if [ -f "${LOGROTATE_DIR}/llm-quotation" ]; then
        rm "${LOGROTATE_DIR}/llm-quotation"
...
health_check() {
    ...
    if curl -s --connect-timeout 5 "$HEALTH_URL" | grep -q "healthy"; then
        print_success "基础健康检查: 通过"
    else
        print_warning "基础健康检查: 失败 (服务可能未运行)"
        return 1
    fi
    ...
    echo ""
    print_info "详细健康状态:"
    curl -s "$HEALTH_DETAILED_URL" | python3 -m json.tool 2>/dev/null || \
        curl -s "$HEALTH_DETAILED_URL"
    echo ""
}
...
full_deploy() {
    ...
    if [ -d "alembic/versions" ] && [ "$(ls -A alembic/versions 2>/dev/null)" ]; then
        print_info "运行数据库迁移..."
        alembic upgrade head 2>/dev/null || print_warning "迁移跳过"
    fi
    ...
    echo "  - 详细健康: http://localhost:8000/health/detailed"
    echo ""
    print_info "管理命令:"
